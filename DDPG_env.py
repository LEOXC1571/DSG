...
class DGEnv(gym.Env):
    ...
    def __init__(self, cfg, deflection_angle=0.5 * math.pi, is_continue=False):  # 初始化环境
        ...
        if is_continue:
            self.action_space = spaces.Box(low=-0.5*math.pi, high=0.5*math.pi, shape=(1,))
        else:
    ...
    def step(self, action):
        ...
        dog_angle = angle_normalize(dog_angle)
        goat_angle = angle_normalize(goat_angle)
        dog_initx = dog_x
        dog_inity = dog_y
        goat_initx = goat_x
        goat_inity = goat_y
        ...
        # goat_next_temp_angle = goat_angle + deflection_angle
        tune_angle = self.goat_next_angle(deflection_angle,(goat_x,goat_y))
        ...
        goat_next_x, goat_next_y = polar2cartesian((goat_x, goat_y), self.goat_spd, goat_next_angle)
        # goat_next_x, goat_next_y = self.goat_next_pos((goat_x,goat_y),deflection_angle,goat_next_angle)
        ...
        def ts_reward():
            ...
            if not done:  # 游戏没有结束 done = False
                if self.step_not_got_goat is None:  # 如果是第一步
                    ...
                    # reward = to_strategy(x , self.dog_spd / self.goat_spd)/30
                    # if deflection_angle > math.pi/2 or deflection_angle < -math.pi/2:
                    #     reward+= -10
                    # if deflection_angle > -math.pi*3/4 and deflection_angle < math.pi*3/4:
                    #     reward+= 1
                    reward=0
    ...
    def _dog_pursuit_policy(self, dog_angle, goat_angle) -> int:
        ...
        if (0 < goat_dog_diff <= math.pi) or (-2 * math.pi < goat_dog_diff <= -math.pi):
            return 0
        elif (-math.pi < goat_dog_diff) or (math.pi < goat_dog_diff < 2 * math.pi):
            return 1
        elif goat_dog_diff == 0:
            return 2
        else:
    ...
    def goat_next_pos(self, pos, deflection, new_angle):
        h=self.goat_spd * abs(math.sin(deflection))
        x1=self.goat_spd * math.cos(deflection)
        x0=eucliDist(pos,(self.center_x,self.center_y))
        h0=x1+x0
        r=math.sqrt(h**2+h0**2)
