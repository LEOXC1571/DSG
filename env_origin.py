...
class DGEnv(gym.Env):
    ...
    def __init__(self, cfg, deflection_angle=0.25 * math.pi, is_continue=False):  # 初始化环境
        # 固定游戏区域
        self.game_size = 500
        self.center_x = 250
        self.center_y = 250
    ...
    def step(self, action):
        ...
        if dog_next_decision == 0:  # 顺时针
            dog_next_angle = dog_angle + dog_angle_spd
        elif dog_next_decision == 1:  # 逆时针
            dog_next_angle = dog_angle - dog_angle_spd
        else:
        ...
        if type(action) is type(1):  # 离散 DQN
            diff_center_goat_old = eucliDist((self.center_x, self.center_y),
                                     (goat_x, goat_y))
            deflection_angle = self.deflection_angle * 1/(diff_center_goat_old/ self.radius) # 越接近边缘角度越小
            if action == 0:  # 未修正的角度
